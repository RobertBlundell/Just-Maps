...
class Onboarding_3: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "onboardingPlaceCell", for: indexPath) as? onboardingPlaceCell {
            ...
            cell.configureCellVisuals(place: Walks[indexPath.row])
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        cell?.layer.borderWidth = 2.0
        cell?.layer.borderColor = UIColor(red: 122/255, green: 191/255, blue: 47/255, alpha: 1).cgColor
        let cellKey = Walks[indexPath.row].title
